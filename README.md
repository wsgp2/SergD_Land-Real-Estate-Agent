...
| **Маршрутизация** | [**OSRM**](https://project-osrm.org) (публичный сервер `routing.openstreetmap.de`) — реальное автомобильное время по графу OSM. Заменил грубую формулу `haversine × road_factor` (которая в Екатеринбурге давала «5 минут» там где на машине 12-15). Без ключа, без лимитов на разумные объёмы, rate limit 1 req/s. Не учитывает пробки — для пробок нужен Yandex Routing (платный). |
...
# 5. Реальное время в пути через OSRM (заменяет грубую формулу haversine)
python scripts/recalc_drive_osrm.py

# 6. Финальный отчёт + Leaflet карта + JSON шорт-лист
python scripts/report.py

# 7. Экспорт топа для Telegram (HTML с тегами + plain с Unicode-bold)
...
├── recalc_drive_osrm.py     # этап 7: реальное drive_time через OSRM по графу OSM
...
ВОРОНКА:
  ...
  Реальное drive_time через OSRM          279 (1 не маршрутизирован)
  В 25-минутной зоне от якоря             139  ← реальные минуты по OSM
  В 15 минутах                             36
  Чистая группа A (6-14 сот, ИЖС/ЛПХ/СНТ) 24-77 в зависимости от фильтра
  Топ-15 мин ИЖС без коммерции             5  ← финальный шорт-лист
```

⚠ До OSRM формула `haversine × 1.35 / 50км/ч` давала **в 2 раза заниженные минуты**
для городских маршрутов в Екб — слишком много мостов через Исеть и ЖД-узлов,
которые формула игнорирует. После OSRM 25-минутная зона ужалась со 153 до 139 листингов
(минус ложные «5-минутки»), 15-минутная — с 77 до 36.
